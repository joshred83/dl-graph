...
from dominant import DOMINANTAugmented  # note we're using the custom DOMINANTAugmented class here
...
def create_loader(data:EllipticBitcoinDataset, num_workers=4, batch_size=2048, num_neighbors=[10, 10], use_train_mask=True) -> NeighborLoader:
    """
...
def train_model(model:DOMINANTAugmented, loader:NeighborLoader, 
    ...
    for epoch in tqdm(range(num_epochs), desc="Epochs"):
        ...
        for i, batch in tqdm(enumerate(loader), desc=f"Epoch {epoch+1}", leave=True):
            ...
            optimizer.zero_grad()
            x_hat, s_hat = model(x, edge_index, apply_augmentation=True) # this is working fine currently but we may need to change if we don't use augmentation
    ...
    with open(metrics_path, 'w') as f:
        json.dump({k: [float(v) for v in vals] for k, vals in metrics.items()}, f) # apparently json doesn't like torch tensors
...
def test_model(model:DOMINANTAugmented, data, device, batch_size=2048, num_neighbors=[10,10], output_directory='./outputs', threshold=0.5) -> Dict[str, float]:
    ...
    if model.use_aggregation:
        print(f"Aggregation methods - Mean: {model.aggregation_mean}, Max: {model.aggregation_max}")
...
def plot_loss(loss_history, attr_loss_history, struct_loss_history, output_directory='./outputs'):
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(range(1, len(attr_loss_history)+1), attr_loss_history, 'o-', label='Attribute Loss')
    plt.plot(range(1, len(struct_loss_history)+1), struct_loss_history, 'o-', label='Structure Loss')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.yscale('log')
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_directory, f'attr_struct_loss_plot_{timestamp}.png'))
    plt.close()
